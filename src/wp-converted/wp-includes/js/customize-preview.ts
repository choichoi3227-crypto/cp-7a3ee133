// Converted from customize-preview.js (WordPress official source → TypeScript)
// Source: https://github.com/WordPress/WordPress

// @ts-ignore - WordPress globals
declare const wp: any;

// @ts-ignore - jQuery type
...
(function( exports, $ ){
	...
	( function( history ) {
		...
		injectUrlWithState = function( url ) {
			let urlParser, oldQueryParams, newQueryParams;
	...
	api.Preview = api.Messenger.extend(/** @lends wp.customize.Preview.prototype */{
		...
		initialize: function( params, options ) {
			let preview = this, urlParser = document.createElement( 'a' );
		...
		handleLinkClick: function( event ) {
			let preview = this, link, isInternalJumpLink;
		...
		handleFormSubmit: function( event ) {
			let preview = this, urlParser, form;
			urlParser = document.createElement( 'a' );
			form = $( event.target );
	...
	api.addLinkPreviewing = function addLinkPreviewing() {
		let linkSelectors = 'a[href], area[href]';
	...
	api.isLinkPreviewable = function isLinkPreviewable( element, options ) {
		let matchesAllowedUrl, parsedAllowedUrl, args, elementHost;
	...
	api.prepareLinkPreview = function prepareLinkPreview( element ) {
		let queryParams, $element = $( element );
	...
	api.addRequestPreviewing = function addRequestPreviewing() {
		 ...
		 * @return {void}
		 */
		let prefilterAjax = function( options, originalOptions, xhr ) {
			let urlParser, queryParams, requestMethod, dirtyValues = {};
			urlParser = document.createElement( 'a' );
	...
	api.keepAliveCurrentUrl = ( function() {
		...
		return function keepAliveCurrentUrl() {
			let urlParser, currentQueryParams;
	...
	api.settingPreviewHandlers = {
		...
		custom_css: function( value ) {
			...
			if ( api.settings.theme.isBlockTheme ) {
				...
				let textContent = style.text().replace(
	...
	$( function() {
		let bg, setValue, handleUpdatedChangesetUuid;
